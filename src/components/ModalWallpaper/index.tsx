...
interface ModalWallpaperProps {
    ...
    srcFile?: (file: File) => void;
    language?: any;
}

const ModalWallpaper: React.FC<ModalWallpaperProps> = ({ onSelect, srcFile, language }) => {

    const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.files && e.target.files[0] && srcFile) {
    ...
    return (
        <Container>
            <h1 style={{ color: '#fff' }}>{language === 'PT-BR' ? 'Selecione o Wallpaper' : 'Select Wallpaper'}</h1>
            ...
            <div className="input-select-file">
                <label htmlFor="file" className="custum-file-upload">
                    ...
                    <div className="text">
                        <span>{language === 'PT-BR' ? 'Escolha o próprio Wallpaper' : 'Choose your own Wallpaper'}</span>
                    </div>
